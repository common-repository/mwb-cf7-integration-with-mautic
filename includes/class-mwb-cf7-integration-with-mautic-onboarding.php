 ...
 */
if ( class_exists( 'Mwb_Cf7_Integration_With_Mautic_Onboarding' ) ) {
	return;
}
 ...
 */
class Mwb_Cf7_Integration_With_Mautic_Onboarding {
	...
	public function __construct() {
		...
		if ( defined( 'MWB_CF7_MAUTIC_ONBOARD_PLUGIN_NAME' ) ) {
			self::$plugin_name = MWB_CF7_MAUTIC_ONBOARD_PLUGIN_NAME;
		}
	...
	public function enqueue_styles() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_style( 'makewebbetter-onboarding-style', MWB_CF7_INTEGRATION_WITH_MAUTIC_URL . 'admin/css/makewebbetter-onboarding-admin.css', array(), '1.0.0', 'all' );

			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_style( 'makewebbetter-onboarding-select2-style', MWB_CF7_INTEGRATION_WITH_MAUTIC_URL . 'packages/select2/select2.min.css', array(), '1.0.0', 'all' );
		}
	}
	...
	public function enqueue_scripts() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_script( 'makewebbetter-onboarding-scripts', MWB_CF7_INTEGRATION_WITH_MAUTIC_URL . 'admin/js/makewebbetter-onboarding-admin.js', array( 'jquery' ), '1.0.0', true );
			...
			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_script( 'makewebbetter-onboarding-select2-script', MWB_CF7_INTEGRATION_WITH_MAUTIC_URL . 'packages/select2/select2.min.js', array( 'jquery' ), '1.0.0', false );
		}
	}
	...
	public function add_onboarding_popup_screen() {
		...
		if ( $this->is_valid_page_screen() && $this->can_show_onboarding_popup() ) {
			require_once MWB_CF7_INTEGRATION_WITH_MAUTIC_DIRPATH . 'extra-templates/makewebbetter-onboarding-template-display.php';
		}
	}
	...
	public function add_deactivation_popup_screen() {
		...
		if ( ! empty( $pagenow ) && 'plugins.php' == $pagenow ) { // phpcs:ignore
			require_once MWB_CF7_INTEGRATION_WITH_MAUTIC_DIRPATH . 'extra-templates/makewebbetter-deactivation-template-display.php';
		}
	}
	...
	public function add_on_boarding_form_fields() {
		...
		$fields = array(
			...
			wp_rand() => array(
				'id'          => 'monthly-revenue',
				'label'       => esc_html__( 'What is your monthly revenue?', 'mwb-cf7-integration-with-mautic' ),
				'type'        => 'radio',
			...
			wp_rand() => array(
				'id'          => 'industry_type',
				'label'       => esc_html__( 'What industry defines your business?', 'mwb-cf7-integration-with-mautic' ),
				'type'        => 'select',
				'name'        => 'industry_type_',
				'value'       => '',
				'multiple'    => 'yes',
			...
			wp_rand() => array(
				'id'          => 'onboard-email',
				'label'       => esc_html__( 'What is the best email address to contact you?', 'mwb-cf7-integration-with-mautic' ),
				'type'        => 'email',
				'name'        => 'email',
			...
			wp_rand() => array(
				'id'          => 'onboard-number',
				'label'       => esc_html__( 'What is your contact number?', 'mwb-cf7-integration-with-mautic' ),
				'type'        => 'text',
				'name'        => 'phone',
				'value'       => '',
				'required'    => 'yes',
	...
	public function add_deactivation_form_fields() {
		...
		$fields = array(
			...
			wp_rand() => array(
				...
				'extra-class' => '',
				'options'     => array(
					'temporary-deactivation-for-debug' => esc_html__( 'It is a temporary deactivation. I am just debugging an issue.', 'mwb-cf7-integration-with-mautic' ),
					'site-layout-broke'                => esc_html__( 'The plugin broke my layout or some functionality.', 'mwb-cf7-integration-with-mautic' ),
					'complicated-configuration'        => esc_html__( 'The plugin is too complicated to configure.', 'mwb-cf7-integration-with-mautic' ),
					'no-longer-need'                   => esc_html__( 'I no longer need the plugin', 'mwb-cf7-integration-with-mautic' ),
					'found-better-plugin'              => esc_html__( 'I found a better plugin', 'mwb-cf7-integration-with-mautic' ),
					'other'                            => esc_html__( 'Other', 'mwb-cf7-integration-with-mautic' ),
				),
			),
	...
	private function hic_post( $endpoint, $post_params, $headers ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$status_code = 500;
			$_response   = esc_html__( 'Unexpected Error Occured', 'mwb-cf7-integration-with-mautic' );
			$errors      = $response;
