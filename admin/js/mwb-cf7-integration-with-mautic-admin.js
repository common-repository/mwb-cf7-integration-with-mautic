...
jQuery( document ).ready( function() {
	...
	const crm          = mwb_cf7_ajax_data.crm
	const ajaxUrl      = mwb_cf7_ajax_data.ajaxUrl;
	const nonce        = mwb_cf7_ajax_data.ajaxNonce;
	const action       = mwb_cf7_ajax_data.ajaxAction;
	const error        = mwb_cf7_ajax_data.criticalError;
	const feedBackLink = mwb_cf7_ajax_data.feedBackLink;
	const feedBackText = mwb_cf7_ajax_data.feedBackText;
	const isPage       = mwb_cf7_ajax_data.isPage;
	const trashIcon    = mwb_cf7_ajax_data.trashIcon;
	const apiKeyImg    = mwb_cf7_ajax_data.apiKeyImg;
	const webtoImg     = mwb_cf7_ajax_data.webtoImg;
	const adminUrl     = mwb_cf7_ajax_data.adminUrl;
	...
	const initiliazeFeedForm = () => {
		...
		jQuery('#mwb_mautic_cf7_feeds_associate_user_meta_box').hide();
		jQuery('#mwb_mautic_cf7_feeds_associate_segment_meta_box').hide();
		jQuery('#mwb_mautic_cf7_feeds_associate_campaign_meta_box').hide();
		jQuery('#mwb_mautic_cf7_feeds_associate_org_meta_box').hide();
		jQuery('#mwb_mautic_cf7_feeds_add_note_meta_box').hide();
		jQuery('#mwb_mautic_cf7_feeds_add_tag_meta_box').hide();
		jQuery('#mwb_mautic_cf7_feeds_add_attachment_meta_box').hide();
		jQuery('#mwb_mautic_cf7_feeds_manual_sync_meta_box').hide();
		...
		if ( '-1' != object && '-1' != form && '' != post_id && 'mwb_' + crm + '_cf7' == post_type ) {
			if ( 'publish' == post_status ) {
				...
				if( object == 'contact' ) {
					jQuery('#mwb_mautic_cf7_feeds_associate_user_meta_box').show();
					jQuery('#mwb_mautic_cf7_feeds_associate_segment_meta_box').show();
					jQuery('#mwb_mautic_cf7_feeds_associate_campaign_meta_box').show();
					jQuery('#mwb_mautic_cf7_feeds_associate_org_meta_box').show();
					jQuery('#mwb_mautic_cf7_feeds_add_note_meta_box').show();
					jQuery('#mwb_mautic_cf7_feeds_add_tag_meta_box').show();
					jQuery('#mwb_mautic_cf7_feeds_add_attachment_meta_box').show();
					jQuery('#mwb_mautic_cf7_feeds_manual_sync_meta_box').show();
				} else {
					jQuery('#mwb_mautic_cf7_feeds_associate_user_meta_box').show();
					jQuery('#mwb_mautic_cf7_feeds_associate_segment_meta_box').hide();
					jQuery('#mwb_mautic_cf7_feeds_associate_campaign_meta_box').hide();
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				if ( response.data.hasOwnProperty( 'html' ) ) {
					...
					jQuery('.mwb-feed__select-object').closest( 'div.mwb_' + crm + '_cf7__feeds-wrap' ).parent().append( response.data.html );
	 ...
	 */
	 const applySelect2 = () => {
		if ( jQuery( 'body' ).hasClass( 'contact_page_mwb_' + crm + '_cf7_page' ) ) {
			if ( jQuery( '#mwb_setting_dont_save' ).length > 0 ) {
				jQuery( '#mwb_setting_dont_save' ).select2( {
	 ...
	 */
	 const createAddNoteField = () => {
		...
		html += '<select name="add-note-select" id="mwb-'+ crm +'-add-note-select">';
		html += '<option value="-1">Select field</option>'
	...
	jQuery( '#mwb-mautic_cf7-auth_type' ).on( 'change', function() {
	...
	jQuery( '#mwb-showauth-form' ).on( 'click', function( e ) {
		...
		jQuery( '.mwb-intro' ).slideUp( '400', function() {
			jQuery( '.mwb_sf_cf7__account-wrap' ).slideDown();
		} );
	} );

	/* Show setup documentation */
	jQuery( document ).on( 'click', '.trigger-setup-guide', function() {
		var intMethod = jQuery( '#mwb-mautic_cf7-auth_type' ).val();
	...
	jQuery( '.mwb-section__sub-heading__wrap' ).on( 'click', function() {
		...
		jQuery( this ).siblings( '.mwb-sf_cf7__status-wrap' ).slideToggle( 300 );
	} );
	...
	jQuery( '#mwb-' + crm + '_cf7-authorize-button' ).on( 'click', function( e ) {
		e.preventDefault();
		
		href            = jQuery( this ).attr( 'href' );
		consumer_key    = jQuery( '#mwb-' + crm + '_cf7-consumer-key' ).val();
		consumer_secret = jQuery( '#mwb-' + crm + '_cf7-consumer-secret' ).val();
		auth_type       = jQuery( '#mwb-' + crm + '_cf7-auth_type' ).val();
		redirect_url    = jQuery( '#mwb-' + crm + '_cf7-redirect-url' ).val();
		username        = jQuery( '#mwb-' + crm + '_cf7-username' ).val();
		password        = jQuery( '#mwb-' + crm + '_cf7-password' ).val();
	...
	jQuery( '#mwb_' + crm + '_cf7_refresh_token' ).on( 'click', function( e ) {
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				if ( true == response.data.success ) {
					...
					jQuery( '#mwb-' + crm + '_cf7-token-notice' ).text( response.data.connection_message );
					jQuery( '#mwb-' + crm + '_cf7-token-notice' ).append( '<br>' );
					jQuery( '#mwb-' + crm + '_cf7-token-notice' ).append( response.data.token_message );
				}
			} else {
	...
	jQuery( '#mwb_' + crm + '_cf7_reauthorize' ).on( 'click', function( e ) {
		e.preventDefault();
	...
	jQuery( '#mwb_' + crm + '_cf7_revoke' ).on( 'click', function( e ) {
		e.preventDefault();
		let event = 'revoke_' + crm + '_cf7_access';
		let input = {
			action: action,
			nonce : nonce,
			event : event,
	...
	jQuery( '#mwb-' + crm + '-cf7-select-form' ).on( 'change', function() {
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				...
				jQuery( '#mwb-' + crm + '-cf7-add-tags' ).slideUp();
				let note = createAddNoteField();
				let tag = createAddTagField();
				jQuery( '#mwb-' + crm + '-cf7-feed-add-note' ).html( note );
				jQuery( '#mwb-' + crm + '-cf7-feed-add-note' ).slideDown();
				jQuery( '#mwb-' + crm + '-cf7-add-tags' ).html( tag );
				jQuery( '#mwb-' + crm + '-cf7-add-tags' ).slideDown();
			} else {
				triggerError();
			}
		} );
	...
	jQuery( document ).on( 'change', '#mwb-'+ crm +'-add-note-select', function( e ) {
		let type_select = jQuery( this );
		let field_value = type_select.val();
		let input       = jQuery( '#mwb-' + crm + '-add-feed-note' ).val();

		jQuery( '#mwb-' + crm + '-add-feed-note' ).val( input + '{' + field_value + '}' );
	} );
	...
	jQuery( document ).on( 'change', '#mwb-' + crm + '-add-tag-select', function( e ) {
		let type_select = jQuery( this );
		let field_value = type_select.val();
		let input       = jQuery( '#mwb-' + crm + '-add-tags' ).val();

		jQuery( '#mwb-' + crm + '-add-tags' ).val( input + '{' + field_value + '}' );
	} );
	...
	jQuery( '.mwb_' + crm + '_cf7__trash_feed' ).on( 'click', function() {
		...
		result.then( ( response ) => {
			if ( true == response.status ) {
				if ( true == response.data.status ) {
					jQuery( '#mwb-cf7-independent' ).slideUp();
					jQuery( '#mwb-cf7-independent' ).html( response.data.feeds );
					jQuery( '#mwb-cf7-independent' ).slideDown();
				}
			} else {
	...
	/* Toggle feeds fields */
	jQuery( '.mwb_' + crm + '_cf7__feeds-wrap .mwb-feeds__meta-box-main-wrapper' ).slideDown();
	jQuery( '.mwb_' + crm + '_cf7__feeds-wrap .mwb-feeds__header-link' ).addClass( 'link-active' );
